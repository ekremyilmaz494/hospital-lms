...
import {
  Library, Clock, Star, CheckCircle2, Plus, X,
  Film, Search,
...
// ── Medya Kütüphanesi Tipleri ──────────────────────────────────────────────
//
// my-videos API'si bu kuruma yüklenmiş tüm ContentLibrary öğelerini döndürür.
// Eğitime atanmış olanlar usedInTrainings dizisinde gelir; atanmamışlarda boş.
...
interface MyVideoLibraryItem extends ContentLibraryItem {
  usageCount: number
...
//
// Kuruma yüklenmiş tüm ContentLibrary öğelerini listeler. Eğitime atanmamış
// olanlar dahil — kart üzerindeki "Eğitimde N" rozeti 0 olabilir. Eğitim atama
// burada YAPILMAZ; atama akışı admin/trainings tarafındadır.

function MyVideosTab() {
  const { toast } = useToast()
  const { data, isLoading, error, refetch } = useFetch<MyVideosData>('/api/admin/content-library/my-videos')
  const [categoryFilter, setCategoryFilter] = useState<string | null>(null)
  const [searchQuery, setSearchQuery] = useState('')
  const [showUploadModal, setShowUploadModal] = useState(false)
  const [deleteTarget, setDeleteTarget] = useState<MyVideoLibraryItem | null>(null)
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p style={{ marginTop: 4, fontSize: 13, color: K.TEXT_MUTED }}>
            Kurumunuza yüklenmiş tüm içerikler — {allItems.length} öğe · {linkedTrainingIds.size} eğitime bağlı · {totalDurationMin}dk
          </p>
        </div>
        <button
          ...
          description={
            searchQuery
              ...
              : categoryFilter
                ...
                : '"Yeni İçerik Yükle" butonuyla ilk videonuzu ekleyin. Sonra bu içeriği bir eğitime atayabilirsiniz.'
          }
...
function MyVideoCard({ item, onDelete, deleting }: MyVideoCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-3" style={{ padding: 16 }}>
        ...
        {/* Bottom action row: linked training + delete */}
        <div className="mt-auto pt-1 flex items-stretch gap-2">
          {primaryTraining ? (
            <span
            ...
            <span
              ...
              style={{
                ...
                color: K.TEXT_MUTED,
                background: K.BG,
                border: `1.5px dashed ${K.BORDER_LIGHT}`,
                borderRadius: 10,
              }}
            >
              <GraduationCap size={13} style={{ color: K.TEXT_MUTED, flexShrink: 0 }} />
              <span>Hiçbir eğitime atanmadı</span>
            </span>
          )}
          {item.usageCount === 0 && (
            <button
              ...
              onMouseEnter={(e) => { if (!deleting) { e.currentTarget.style.background = K.ERROR_BG; e.currentTarget.style.borderColor = K.ERROR; e.currentTarget.style.color = K.ERROR } }}
              onMouseLeave={(e) => { e.currentTarget.style.background = K.SURFACE; e.currentTarget.style.borderColor = K.BORDER; e.currentTarget.style.color = K.TEXT_MUTED }}
            >
              <Trash2 size={14} />
            </button>
          )}
      ...
      <header className="k-page-header">
        <div>
          <div className="k-breadcrumb">
            ...
            <span data-current="true">Medya Kütüphanesi</span>
          </div>
          <h1 className="k-page-title">Medya Kütüphanesi</h1>
          <p className="k-page-subtitle">Kuruma yüklediğiniz tüm video, ses ve doküman içerikleri</p>
        </div>
        <div
          className="flex items-center justify-center"
          style={{
