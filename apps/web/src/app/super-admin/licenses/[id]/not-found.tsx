import Link from 'next/link';
import { KeyRound } from 'lucide-react';
import { Button } from '@/components/ui/button';

export default function NotFound() {
  return (
    <div className="flex min-h-[60vh] items-center justify-center p-8">
      <div className="text-center">
        <h1
          className="text-7xl font-bold mb-4"
          style={{
            background: 'linear-gradient(135deg, var(--color-primary), var(--color-accent))',
            WebkitBackgroundClip: 'text',
            WebkitTextFillColor: 'transparent',
          }}
        >
          404
        </h1>
        <h2 className="text-xl font-bold mb-2">Lisans Bulunamadı</h2>
        <p className="text-sm mb-8 max-w-sm mx-auto" style={{ color: 'var(--color-text-secondary)' }}>
          Aradığınız lisans mevcut değil veya silinmiş olabilir.
        </p>
        <div className="flex items-center justify-center gap-3">
          <Link href="/super-admin/licenses">
            <Button className="gap-2 font-semibold text-white" style={{ background: 'var(--color-primary)' }}>
              <KeyRound className="h-4 w-4" /> Lisans Listesi
            </Button>
          </Link>
        </div>
      </div>
    </div>
  );
}
