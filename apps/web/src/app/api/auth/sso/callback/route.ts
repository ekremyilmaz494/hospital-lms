...
export async function GET(request: NextRequest) {
  ...
  // SSO callback redirect'leri kullanıcıya özel state içerir — cache YASAK.
  const noCacheHeaders = { 'Cache-Control': 'no-store' }

  if (!code || !stateParam) {
    return NextResponse.redirect(new URL('/auth/login?error=sso_failed', request.url), { headers: noCacheHeaders })
  }
